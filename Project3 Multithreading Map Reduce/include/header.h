...
struct Res
{   //result histogram shared by consumers
  int letter[20];
...
struct Queue 
{   //shared queue where consumers get data from the head
    struct QNode *front, *rear;//producer put data in the tail
    int size;//restricted by the bound if -b flag exists
};

struct condBuffer 
{   //shared information buffer used by consumers and producer
